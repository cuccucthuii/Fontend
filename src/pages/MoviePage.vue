  ...
  <div class="movie-page">
    ...
    <div class="table-section">
      ...
      <div class="table-container">
        ...
        <tbody>
            <tr v-for="(movie, index) in paginatedMovies" :key="movie.idPhim" class="table-row">
              ...
              <td class="td-status">
                <span class="status-badge" :class="getStatusClass(movie.trangThai)">
    ...
    <div class="pagination-section" v-if="totalPages > 1">
      <div class="pagination-info">
        <span class="pagination-text">
          Hiển thị {{ (currentPage - 1) * pageSize + 1 }} - {{ Math.min(currentPage * pageSize, filteredMovies.length) }} 
          trong tổng số {{ filteredMovies.length }} phim
        </span>
      </div>
          ...
          <form @submit.prevent="submitEditForm" class="edit-form">
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input id="edit-duration" type="number" v-model="editForm.thoiLuong" required />
              </div>
  </div>

            <div class="form-group">
              <label for="edit-description">Mô tả</label>
              <textarea id="edit-description" v-model="editForm.moTa" rows="3"></textarea>
            </div>
            
            <div class="form-row">
              <div class="form-group">
                <label for="edit-release">Ngày phát hành</label>
                <input id="edit-release" type="date" v-model="editForm.ngayPhatHanh" />
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input id="edit-category" v-model="editForm.theLoai" placeholder="Phân cách bằng dấu phẩy" />
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input id="edit-director" v-model="editForm.daoDien" placeholder="Phân cách bằng dấu phẩy" />
              </div>
              <div class="form-group">
                <label for="edit-actors">Diễn viên</label>
                <input id="edit-actors" v-model="editForm.dienVien" placeholder="Phân cách bằng dấu phẩy" />
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input id="edit-poster" v-model="editForm.posterUrl" placeholder="Đường dẫn ảnh poster" />
              </div>
              <div class="form-group">
                <label for="edit-banner">URL Banner</label>
                <input id="edit-banner" v-model="editForm.bannerUrl" placeholder="Đường dẫn ảnh banner" />
              </div>
            </div>
            
            <div class="form-group">
              <label for="edit-trailer">URL Trailer</label>
              <input id="edit-trailer" v-model="editForm.trailerUrl" placeholder="Đường dẫn video trailer" />
            </div>
      ...
      <div v-if="showDeleteModal" class="modal-overlay" @click.self="closeDeleteModal">
        <div class="modal-card modal-danger">
          ...
          <div class="delete-content">
            ...
            <div class="movie-preview" v-if="movieToDelete">
              ...
              <div class="preview-info">
                ...
                <p>{{ movieToDelete.thoiLuong }} phút • {{ movieToDelete.dinhDang }}</p>
                <p class="preview-status" :class="getStatusClass(movieToDelete.trangThai)">
          ...
          <div class="modal-actions">
            ...
            <button class="btn btn-danger" @click="confirmDeleteMovie" :disabled="isLoading">
              <span v-if="isLoading" class="loading-spinner-btn"></span>
              Xóa vĩnh viễn
            </button>
          </div>
...
const filteredMovies = computed(() => {
  ...
  if (statusFilter.value) {
    filtered = filtered.filter(movie => movie.trangThai === statusFilter.value)
  }
  
  // Category filter
  if (categoryFilter.value) {
    filtered = filtered.filter(movie => 
      Array.isArray(movie.theLoai) && movie.theLoai.includes(categoryFilter.value)
    )
  }
  
  // Format filter
  if (formatFilter.value) {
    filtered = filtered.filter(movie => movie.dinhDang === formatFilter.value)
  }
...
const hasActiveFilters = computed(() => {
  return statusFilter.value || categoryFilter.value || formatFilter.value || searchQuery.value
})
...
async function loadMovies() {
  try {
    ...
    // Hỗ trợ cả 2 dạng trả về: mảng trực tiếp hoặc dạng phân trang { content: [...] }
    if (Array.isArray(response.data)) {
      movies.value = response.data
    } else if (Array.isArray(response.data?.content)) {
      movies.value = response.data.content
    } else {
      console.warn('⚠️ Dữ liệu phim không đúng định dạng mong đợi:', response.data)
      movies.value = []
    }
...
function exportData() {
  ...
  try {
    ...
    link.setAttribute('download', `danh-sach-phim-${new Date().toISOString().split('T')[0]}.xlsx`)
