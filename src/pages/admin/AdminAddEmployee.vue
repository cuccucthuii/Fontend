  ...
  <div class="add-employee-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-subtitle">Thêm nhân viên mới vào hệ thống quản lý rạp chiếu phim</p>
      </div>
    </div>
    ...
    <div class="form-container">
      <div class="form-card">
        <div class="form-header">
          ...
          <p class="form-description">Vui lòng điền đầy đủ thông tin bên dưới</p>
        </div>
          ...
          <div class="form-section">
            ...
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                >
                  <option value="">
                    {{ isLoadingCinemas ? '🔄 Đang tải danh sách rạp...' : '-- Chọn rạp chiếu --' }}
                  </option>
                  <option 
...
function validateEmailInput() {
  ...
  if (!emailRegex.test(email)) {
    emailError.value = '❌ Email không đúng định dạng! (VD: example@domain.com)'
    return
  }
  
  // Check for common invalid patterns
  if (email.startsWith('.') || email.endsWith('.') || email.includes('..')) {
    emailError.value = '❌ Email không được bắt đầu hoặc kết thúc bằng dấu chấm!'
    return
  }
...
function validateFullNameInput() {
  ...
  if (!nameRegex.test(fullName)) {
    fullNameError.value = '❌ Họ và tên chỉ được chứa chữ cái, dấu cách và gạch ngang!'
    return
  }
...
async function fetchCinemas() {
  ...
  try {
    ...
    cinemas.value = [
      { idRapChieu: 1, tenRapChieu: 'Rạp CGV Aeon Mall', diaChi: '123 Đường ABC, Quận 1, TP.HCM' },
      { idRapChieu: 2, tenRapChieu: 'Rạp CGV Crescent Mall', diaChi: '456 Đường XYZ, Quận 7, TP.HCM' },
      { idRapChieu: 3, tenRapChieu: 'Rạp CGV Landmark 81', diaChi: '789 Đường DEF, Quận Bình Thạnh, TP.HCM' }
    ]
  } finally {
...
async function handleAddEmployee() {
  ...
  if (fullNameError.value || genderError.value || cccdError.value || birthdayError.value || startDateError.value || cinemaIdError.value) {
  ...
  try {
    ...
    const emailCheckResponse = await axios.get(`http://localhost:8080/api/employees/check-email?email=${form.value.email}`, {
      headers: {
    ...
    const cccdCheckResponse = await axios.get(`http://localhost:8080/api/employees/check-cccd?cccd=${form.value.cccd}`, {
      headers: {
        Authorization: token ? `Bearer ${token}` : undefined
      }
    })
    ...
    const phoneCheckResponse = await axios.get(`http://localhost:8080/api/employees/check-sdt?soDienThoai=${form.value.phone}`, {
      headers: {
        Authorization: token ? `Bearer ${token}` : undefined
      }
    })
    ...
    const response = await axios.post('http://localhost:8080/api/employees/addEmployee', {
