...
// Modal states (placeholder, chưa làm modal)
function openAddModal() { showToast('🚧 Chức năng đang phát triển!', 'info') }
function openEditModal(room) { showToast('🚧 Chức năng đang phát triển!', 'info') }
function openDeleteModal(room) { showToast('🚧 Chức năng đang phát triển!', 'info') }
...
const activeRooms = computed(() => rooms.value.filter(r => r.trangThaiPhongChieu === 'Đang hoạt động').length)
const inactiveRooms = computed(() => rooms.value.filter(r => r.trangThaiPhongChieu === 'Ngừng hoạt động').length)
