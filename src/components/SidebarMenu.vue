<template>
  <nav class="sidebar-menu">
    <div class="sidebar-header">
      <img src="/logo.png" alt="DEV CINEMA" class="sidebar-img-logo" />
      <p class="subtitle">Quản lý hệ thống</p>
    </div>

    <ul class="menu-list">
      <li v-if="canManage" class="menu-item">
...
// Tự động mở menu phim khi đang ở các trang liên quan
watch(() => route.path, (newPath) => {
  if (newPath.includes('/admin/movies') || newPath.includes('/admin/genres')) {
    showMoviesMenu.value = true
  }
}, { immediate: true })

onMounted(() => {
  // Kiểm tra route hiện tại khi component mount
  if (route.path.includes('/admin/movies') || route.path.includes('/admin/genres')) {
    showMoviesMenu.value = true
  }
})
