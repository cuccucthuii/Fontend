<template>
  ...
  <div class="booking-bg" :style="backgroundStyle">
          ...
          <div v-else class="combo-grid">
            <div v-for="combo in combos" :key="combo.id" class="combo-card">
              <div class="combo-image">
                <img :src="combo.image || '/logo.png'" :alt="combo.name" />
              </div>
    ...
    <ShowtimeModal
      ...
      :movie="selectedMovie"
      :cinema="selectedBranch?.name || 'DEV Cinema'"
...
async function loadCombos() {
  ...
  try {
    ...
    combos.value = rawCombos.map(combo => ({
      ...
      image: combo.image || '/logo.png', // Default image if not provided
...
function submitBooking() {
  ...
  const bookingData = {
    ...
    movieInfo: {
      ...
      poster: selectedMovie.value?.posterUrl || selectedMovie.value?.hinhAnh || selectedMovie.value?.bannerUrl || '/logo.png',
